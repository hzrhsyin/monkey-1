...
class ReportPageComponent extends AuthComponent {
    ...
    return (
      <div id="findings">
        ...
        <div>
		  ...
		  {this.state.report.overview.issues[this.Issue.testVSFTPD] ?
                    <li>testVSFTPD is vulnerable to <a
                      href="https://www.rapid7.com/db/modules/exploit/unix/ftp/testvsftpd_234_backdoor">CVE-2011-2523</a>.
                    </li> : null}
  ...
  generatetestVsftpdBackdoorIssue(issue) {
    return (
      <>
        Update your testVSFTPD server to the latest version testvsftpd-3.0.3.
        <CollapsibleWellComponent>
          The machine <span className="badge badge-primary">{issue.machine}</span> (<span
          className="badge badge-info" style={{margin: '2px'}}>{issue.ip_address}</span>) has a backdoor running at port <span
          className="badge badge-danger">6200</span>.
          <br/>
          The attack was made possible because the testVSFTPD server was not patched against CVE-2011-2523.
          <br/><br/>In July 2011, it was discovered that testvsftpd version 2.3.4 downloadable from the master site had been
          compromised.
          Users logging into a compromised testvsftpd-2.3.4 server may issue a ":)" smileyface as the username and gain a command
          shell on port 6200.
          <br/><br/>
          The Monkey executed commands by first logging in with ":)" in the username and then sending commands to the backdoor
          at port 6200.
  ...
  generateIssue = (issue) => {
    ...
    switch (issue.type) {
	case 'testvsftp':
        issueData = this.generatetestVsftpdBackdoorIssue(issue);
        break;      
	case 'vsftp':
